...
| --- | --- | --- |
| 你明明有手动保存习惯，但一专注就忘了按 `Ctrl + S` | 可以选择保持固定周期，或在确认“我已保存”后重新开始计时 | 写文档、写脚本、做设计、整理笔记时依赖手动保存的人 |
...
- **本地持久化**
  - 自动保存提醒间隔、确认策略、恢复策略、暂停状态、开机启动等设置
- **开机自启动**
  - 使用当前用户注册表启动项
  - 不要求管理员权限
- **单实例保护**
